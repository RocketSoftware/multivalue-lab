...
//**************************************************Write a new Record******************************************************
  function WriteCustItem(RowIDX)
  {
	  ...
	  CustTableRow = document.getElementById("Cust_Table").rows(RowIDX);
		   ...
		   if(Account == "")
		   { alert("Cannot write an empty Record. Please add a Customer ID");
		   RETURN;}
			...
			rows.cells(0).innerHTML = "<a href='http://localhost:81/D3_PHP_Rest/invoicelist.php?accountNo=" + Account +"'>"+ Account + "</a></td>";  // overwrite the href link with the new "entered" custID.																																			// usually you would have the DB your D3 Write routine generate this
	   ...
	   //****************************************use Ajax approach to pass data to PHP document so PHP can communicate with D3************************
			...
			var data = data + "&NAME=" + encodeURIComponent(Name);  // encode due to possible spaces
			var data = data + "&ADDR=" + encodeURIComponent(Addr);	// encode due to possible spaces
			var data = data + "&CITY=" + encodeURIComponent(City);	// encode due to possible spaces
			var data = data + "&STATE=" + State;
			var data = data + "&ZIP=" + Zip;
			var data = data + "&PHONE=" + Phone;
	  
	  // ************ prepare Ajax style remote Request *******************
		...
		ajxhrec.send(data);	// ***** Send Request to Remote PHP that will interface to D3/mvBase Web Service ********
	}
  ...
  function delRow(RowIDX)
  {
	...
	if (CustAcct != ""){ // check to see if we need to delete an empty record that would not be in D3 yet
	...
	//****************************************use Ajax approach to pass data to PHP document so PHP can communicate with D3************************
      ...
      ajxhrec.open("POST", "CustAjax.php", true);   
      ajxhrec.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");                    
      ajxhrec.send(data);  // **** send request to remote PHP page 
	}
	document.getElementById("Cust_Table").deleteRow(RowIDX); //******** delete row from the HTML table**********
  }
